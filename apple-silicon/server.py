"""
Apple Silicon (MPS) Pipeline server:
...
Models (all Unsloth where available):
  ...
  - unsloth/Qwen2.5-VL-7B-Instruct-GGUF   (llama-cpp-python with Metal)

NOTE: This version is optimized for Apple Silicon (M1/M2/M3/M4) using the MPS backend.
      - Uses float16 (bfloat16 is unreliable on MPS)
      - Uses CPU generator (MPS generator produces black images)
      - Does NOT use enable_model_cpu_offload() (CUDA-only, crashes on MPS)
      - Requires PYTORCH_ENABLE_MPS_FALLBACK=1 (set below before torch import)
"""
...
# Disable MPS memory watermark to allow large models to use all available memory
...
# Apple Silicon: prefer MPS, fall back to CPU (CUDA not available)
...
# MPS dtype strategy: bfloat16 for transformer/compute (M-series chips have hardware BF16),
# but float32 for VAE to prevent NaN artifacts that cause black images.
# See: https://github.com/Tongyi-MAI/Z-Image/issues/14
...
def _get_generator(seed: int) -> torch.Generator:
    """Return a seeded generator. Always use CPU device for MPS (MPS generator produces black images)."""
    gen_device = "cpu" if DEVICE == "mps" else DEVICE
    return torch.Generator(device=gen_device).manual_seed(seed)


def _cleanup_memory():
    """Free unused memory on MPS/CUDA."""
    gc.collect()
    if DEVICE == "mps":
...
def _load_zimage():
    ...
    log.info("Loading Z-Image Turbo from %s (dtype=%s) ...", ZIMAGE_MODEL_ID, DTYPE)
    ...
    # MPS: upcast VAE to float32 to prevent NaN latents → black images
    if DEVICE == "mps":
        zimage_pipe.vae.to(dtype=VAE_DTYPE)
        zimage_pipe.enable_attention_slicing()
...
def _load_layered():
    ...
    # Fix: the GGUF-quantized addition_t_embedding dequantizes to wrong shape.
    # The GGUF has (3072, 2) BF16 but diffusers loads it as (2, 6144).
    # Force reshape to (2, inner_dim) where inner_dim = num_heads * head_dim = 3072.
    if hasattr(transformer.time_text_embed, "addition_t_embedding"):
        import torch.nn as nn
        emb = transformer.time_text_embed.addition_t_embedding
        weight = emb.weight.data.float()  # dequantize to float32
        inner_dim = transformer.inner_dim  # 24 * 128 = 3072
        # Flatten and reshape to correct (num_embeddings=2, embedding_dim=inner_dim)
        weight_flat = weight.reshape(-1)
        expected_elements = 2 * inner_dim
        weight_correct = weight_flat[:expected_elements].reshape(2, inner_dim)
        new_emb = nn.Embedding(2, inner_dim)
        new_emb.weight = nn.Parameter(weight_correct.to(dtype=DTYPE))
        transformer.time_text_embed.addition_t_embedding = new_emb
        log.info("Fixed addition_t_embedding: %s -> (2, %d)", tuple(weight.shape), inner_dim)
    ...
    # MPS: do NOT use enable_model_cpu_offload() — it crashes on non-CUDA devices.
    # Instead, move to device directly and enable attention slicing for memory savings.
    if DEVICE == "cuda":
        layered_pipe.enable_model_cpu_offload()
    else:
        layered_pipe.to(DEVICE)
        if DEVICE == "mps":
            layered_pipe.enable_attention_slicing()

    layered_pipe.set_progress_bar_config(disable=None)
    log.info("Qwen-Image-Layered (GGUF) loaded on %s", DEVICE)
...
def _load_qwen_vl():
    ...
    log.info("Loading Qwen2.5-VL GGUF from %s (Metal GPU) ...", QWEN_VL_GGUF)
    chat_handler = handler_cls(clip_model_path=QWEN_VL_MMPROJ)
    # n_gpu_layers=-1 offloads all layers to GPU (Metal on macOS, CUDA on Linux)
    qwen_llm = Llama(
    ...
    log.info("Qwen2.5-VL GGUF loaded (Metal GPU acceleration)")
...
app = FastAPI(
    title="Z-Image Turbo + Qwen-Image-Layered GGUF + Qwen2.5-VL GGUF Pipeline (Apple Silicon)",
    description=(
        ...
        "Optimized for Apple Silicon (M1/M2/M3/M4) with MPS backend and Metal GPU acceleration."
    ),
    lifespan=lifespan,
)
...
async def health():
    return {
        ...
        "device": DEVICE,
        "dtype": str(DTYPE),
        "platform": "apple_silicon" if DEVICE == "mps" else DEVICE,
    }
