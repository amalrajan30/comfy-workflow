...
def download_all():
    ...
    if HF_TOKEN:
        print(f"Using HF_TOKEN for authenticated downloads")
    else:
        print("No HF_TOKEN set (downloads may be rate-limited)")

    dl_kwargs = {"local_dir": MODEL_DIR, "token": HF_TOKEN}
    ...
    hf_hub_download(repo_id=vl_repo, filename="Qwen2.5-VL-7B-Instruct-Q4_K_M.gguf", **dl_kwargs)

    print("[2/4] Downloading mmproj-BF16.gguf (~1.35 GB) ...")
    hf_hub_download(repo_id=vl_repo, filename="mmproj-BF16.gguf", **dl_kwargs)
    ...
    hf_hub_download(repo_id=layered_repo, filename="qwen-image-layered-Q4_K_M.gguf", **dl_kwargs)
    ...
    hf_hub_download(repo_id=vae_repo, filename="split_files/vae/qwen_image_layered_vae.safetensors", **dl_kwargs)
